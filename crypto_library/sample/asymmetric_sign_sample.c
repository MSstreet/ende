#include "test_code.h"
#include "edge_crypto.h"

int ecdsa_sample();
int kcdsa_sample();
int ec_kcdsa_sample();
int kcdsa_init_update_final_sample();
int asymmetric_sign_sample() {
    int ret = 0;

//	edge_crypto_change_mode();
//
    ret = kcdsa_sample();
    if (ret != 0) return ret;
    ret = ecdsa_sample();
    if (ret != 0) return ret;
    ret = kcdsa_init_update_final_sample();
    if (ret != 0) return ret;

    ret = rsa_sample();
    if (ret != 0) return ret;

    ret = ecdsa_sample();
    if (ret != 0) return ret;

    ret = ecdsa_sample();
    if (ret != 0) return ret;

	ret = ec_kcdsa_sample();
    if (ret != 0) return ret;

	/*
    ret = kcdsa_sample();
    if (ret != 0) return ret;

    ret = kcdsa_init_update_final_sample();
    if (ret != 0) return ret;
	*/

    return ret;
}

int kcdsa_sample() {
    int ret = 0;

    EDGE_ASYM_KEY_PARAM key_param;
    EDGE_ASYM_SIGN_PARAM param;
    uint8_t input[16] = "kcdsa_test_text";
    uint32_t inputlength = 16;
    uint8_t sign[512] = { 0x00, };
    uint32_t signlength = 0;
    uint8_t public_key[2048] = { 0x00, };
    uint32_t public_keylength = 0;
    uint8_t private_key[2048] = { 0x00, };
    uint32_t private_keylength = 0;

    memset(&key_param, 0, sizeof(EDGE_ASYM_KEY_PARAM));
    key_param.m_algorithm = EDGE_ASYM_ID_KCDSA;
    key_param.dsa.m_pBitLength = 2048;
    key_param.dsa.m_qBitLength = 224;
    key_param.dsa.m_reqGenParam = 0;
    uint8_t p[256] = { , };
    uint32_t plen = 256;
    uint8_t q[28] = { 0x89,0xDB,0xAC,0xE3,0x90,0x33,0xE6,0xEA,0x6F,0x51,0x6C,0x4A,0x85,0x4A,0xA2,0x28,0x2A,0x50,0xA9,0x55,0x0F,0xD9,0xC2,0x4C,0x16,0x70,0x31,0xAF, };
    uint32_t qlen = 28;
    uint8_t g[256] = { , };
    uint32_t glen = 256;
    memcpy(key_param.dsa.m_param.m_p, p, plen); key_param.dsa.m_param.m_pLen = plen;
    memcpy(key_param.dsa.m_param.m_q, q, qlen); key_param.dsa.m_param.m_qLen = qlen;
    memcpy(key_param.dsa.m_param.m_g, g, glen); key_param.dsa.m_param.m_gLen = glen;

    printf("\n##======================    kcdsa_sample() start    ======================##\n");
    do {
        printf("\n *----------------------   edge_asym_gen_keypair()  ------------------------\n");
        ret = edge_asym_gen_keypair(public_key, &public_keylength, private_key, &private_keylength, &key_param);
        if (ret != 0) break;

        //hexdump("private_key", private_key, private_keylength);
        //hexdump("public_key", public_key, public_keylength);

        param.m_asymAlg = EDGE_ASYM_ID_KCDSA;
        param.m_hashAlg = EDGE_HASH_ID_SHA224;
        param.kcdsa.m_domainParam = key_param.dsa.m_param;

        //hexdump("input", input, inputlength);
        printf("\n *----------------------      edge_asym_sign()      ------------------------\n");
        ret = edge_asym_sign(private_key, private_keylength, &param, input, inputlength, sign, &signlength);
        //ret = edge_asym_sign(public_key, public_keylength, &param, input, inputlength, sign, &signlength);
        //ret = edge_asym_sign(public_key, public_keylength, NULL, input, inputlength, sign, &signlength);
        if (ret != 0) {
			printf ("sign fail %d\n", ret);
			break;
		}
        //hexdump("sign", sign, signlength);

        printf("\n *----------------------     edge_asym_verify()     ------------------------\n");
        //ret = edge_asym_verify(private_key, private_keylength, &param, input, inputlength, sign, signlength);
        ret = edge_asym_verify(public_key, public_keylength, &param, input, inputlength, sign, signlength);
        //ret = edge_asym_verify(NULL, public_keylength, &param, input, inputlength, sign, signlength);
        //ret = edge_asym_verify(public_key, public_keylength, &param, input, inputlength, NULL, signlength);
        if (ret != 0) {
            printf("sign verify fail\n");
            break;
        }
        printf("sign verify success\n");

    } while (0);

    printf("\n##======================     kcdsa_sample() end     ======================##\n");

    return ret;
}

int kcdsa_init_update_final_sample() {
    int ret = 0;

    EDGE_ASYM_KEY_PARAM key_param;
    EDGE_ASYM_SIGN_PARAM param;
    uint8_t input[16] = "kcdsa_test_text";
    uint32_t inputlength = 16;
    uint8_t sign[512] = { 0x00, };
    uint32_t signlength = 0;
    uint8_t public_key[2048] = { 0x00, };
    uint32_t public_keylength = 0;
    uint8_t private_key[2048] = { 0x00, };
    uint32_t private_keylength = 0;
    void* ctx = NULL;

    memset(&key_param, 0, sizeof(EDGE_ASYM_KEY_PARAM));
    key_param.m_algorithm = EDGE_ASYM_ID_KCDSA;
    key_param.dsa.m_pBitLength = 2048;
    key_param.dsa.m_qBitLength = 224;
    //    key_param.dsa.m_reqGenParam = 1;
    uint8_t p[256] = { , };
    uint32_t plen = 256;
    uint8_t q[28] = { 0x89,0xDB,0xAC,0xE3,0x90,0x33,0xE6,0xEA,0x6F,0x51,0x6C,0x4A,0x85,0x4A,0xA2,0x28,0x2A,0x50,0xA9,0x55,0x0F,0xD9,0xC2,0x4C,0x16,0x70,0x31,0xAF, };
    uint32_t qlen = 28;
    uint8_t g[256] = { , };
    uint32_t glen = 256;
    memcpy(key_param.dsa.m_param.m_p, p, plen); key_param.dsa.m_param.m_pLen = plen;
    memcpy(key_param.dsa.m_param.m_q, q, qlen); key_param.dsa.m_param.m_qLen = qlen;
    memcpy(key_param.dsa.m_param.m_g, g, glen); key_param.dsa.m_param.m_gLen = glen;

    printf("\n##================ kcdsa_init_update_final_sample() start ================##\n");
    do {
        printf("\n *----------------------   edge_asym_gen_keypair()  ------------------------\n");
        ret = edge_asym_gen_keypair(public_key, &public_keylength, private_key, &private_keylength, &key_param);
        if (ret != 0) break;

        hexdump("private_key", private_key, private_keylength);
        hexdump("public_key", public_key, public_keylength);

        param.m_asymAlg = EDGE_ASYM_ID_KCDSA;
        param.m_hashAlg = EDGE_HASH_ID_SHA224;
        param.kcdsa.m_domainParam = key_param.dsa.m_param;
        hexdump("input", input, inputlength);

        ctx = edge_asym_sign_ctx_new();
        ret = edge_asym_sign_init(ctx, private_key, private_keylength, &param);
        if (ret != 0) break;

        printf("\n *----------------------     edge_asym_sign_update()   ------------------------\n");
        ret = edge_asym_sign_update(ctx, input, 8);
        if (ret != 0) break;
        ret = edge_asym_sign_update(ctx, input + 8, 8);
        if (ret != 0) break;
        printf("\n *----------------------   edge_asym_sign_final()   ------------------------\n");
        ret = edge_asym_sign_final(ctx, sign, &signlength);
        if (ret != 0) break;
        hexdump("sign", sign, signlength);

        edge_asym_sign_ctx_free(ctx);
        ctx = edge_asym_sign_ctx_new();

        ret = edge_asym_verify_init(ctx, public_key, public_keylength, &param);
        if (ret != 0) break;

        printf("\n *----------------------  edge_asym_verify_update() ------------------------\n");
        ret = edge_asym_verify_update(ctx, input, 8);
        if (ret != 0) break;
        ret = edge_asym_verify_update(ctx, input + 8, 8);
        if (ret != 0) break;

        printf("\n *----------------------  edge_asym_verify_final()  ------------------------\n");
        ret = edge_asym_verify_final(ctx, sign, signlength);
        //ret = edge_asym_verify_final(ctx, NULL, signlength);

        if (ret != 0) {
            printf("sign verify fail\n");
            break;
        }
        printf("sign verify success\n");

    } while (0);
    printf("res -> %d\n", ret);

    edge_asym_sign_ctx_free(ctx);
    printf("\n##================= kcdsa_init_update_final_sample() end =================##\n");

    return ret;
}

int ecdsa_sample() {
    int ret = 0;

    EDGE_ASYM_KEY_PARAM key_param;
    EDGE_ASYM_SIGN_PARAM param;
    uint8_t input[16] = "ecdsa_test_text";
    uint32_t inputlength = 16;
    uint8_t sign[512] = { 0x00, };
    uint32_t signlength = 0;
    uint8_t public_key[2048] = { 0x00, };
    uint32_t public_keylength = 0;
    uint8_t private_key[2048] = { 0x00, };
    uint32_t private_keylength = 0;

    memset(&key_param, 0, sizeof(EDGE_ASYM_KEY_PARAM));
    key_param.m_algorithm = EDGE_ASYM_ID_ECDSA + EDGE_ASYM_ID_ECC_P256;

    printf("\n##======================    ecdsa_sample() start    ======================##\n");
    do {
        printf("\n *----------------------   edge_asym_gen_keypair()  ------------------------\n");
        ret = edge_asym_gen_keypair(public_key, &public_keylength, private_key, &private_keylength, &key_param);
        if (ret != 0) break;

        hexdump("private_key", private_key, private_keylength);
        hexdump("public_key", public_key, public_keylength);

        param.m_asymAlg = EDGE_ASYM_ID_ECDSA + EDGE_ASYM_ID_ECC_P256;
        param.m_hashAlg = EDGE_HASH_ID_SHA256;

        hexdump("input", input, inputlength);
        printf("\n *----------------------      edge_asym_sign()      ------------------------\n");
        ret = edge_asym_sign(private_key, private_keylength, &param, input, inputlength, sign, &signlength);
        //ret = edge_asym_sign(public_key, public_keylength, &param, input, inputlength, sign, &signlength);
        if (ret != 0) {
            printf("sign fail %d\n", ret);
			break;
		}
        hexdump("sign", sign, signlength);

        printf("\n *----------------------     edge_asym_verify()     ------------------------\n");
        ret = edge_asym_verify(public_key, public_keylength, &param, input, inputlength, sign, signlength);
        //ret = edge_asym_verify(public_key, public_keylength, &param, input, inputlength, sign, signlength);
        if (ret != 0) {
            printf("sign verify fail %d\n", ret);
            break;
        }
        printf("sign verify success\n");
    } while (0);
    printf("\n##======================     ecdsa_sample() end     ======================##\n");

    return ret;
}

int ec_kcdsa_sample() {
	int ret = 0;

	EDGE_ASYM_KEY_PARAM key_param;
	EDGE_ASYM_SIGN_PARAM param;
	uint8_t input[19] = "ec_kcdsa_test_text";
	uint32_t inputlength = 19;
	uint8_t sign[512] = { 0x00, };
	uint32_t signlength = 0;
	uint8_t public_key[2048] = { 0x00, };
	uint32_t public_keylength = 0;
	uint8_t private_key[2048] = { 0x00, };
	uint32_t private_keylength = 0;

	memset(&key_param, 0, sizeof(EDGE_ASYM_KEY_PARAM));
	key_param.m_algorithm = EDGE_ASYM_ID_EC_KCDSA + EDGE_ASYM_ID_ECC_P256;

	printf("\n##======================    ec_kcdsa_sample() start    ======================##\n");
	do {
		printf("\n *----------------------   edge_asym_gen_keypair()  ------------------------\n");
		ret = edge_asym_gen_keypair(public_key, &public_keylength, private_key, &private_keylength, &key_param);
		if (ret != 0) break;

		hexdump("private_key", private_key, private_keylength);
		hexdump("public_key", public_key, public_keylength);

		param.m_asymAlg = EDGE_ASYM_ID_EC_KCDSA + EDGE_ASYM_ID_ECC_P256;
		param.m_hashAlg = EDGE_HASH_ID_SHA256;

		hexdump("input", input, inputlength);
		printf("\n *----------------------      edge_asym_sign()      ------------------------\n");
		ret = edge_asym_sign(private_key, private_keylength, &param, input, inputlength, sign, &signlength);
		if (ret != 0) break;
		hexdump("sign", sign, signlength);

		printf("\n *----------------------     edge_asym_verify()     ------------------------\n");
		ret = edge_asym_verify(public_key, public_keylength, &param, input, inputlength, sign, signlength);
		if (ret != 0) {
			printf("sign verify fail\n");
			break;
		}
		printf("sign verify success\n");

	} while (0);

	printf("\n##======================     ec_kcdsa_sample() end     ======================##\n");

	return ret;
}

int rsa_sample() {
    int ret = 0;

    EDGE_ASYM_KEY_PARAM key_param;
    EDGE_ASYM_SIGN_PARAM param;
    uint8_t input[14] = "rsa_test_text";
    uint32_t inputlength = 14;
    uint8_t sign[512] = { 0x00, };
    uint32_t signlength = 0;
    uint8_t public_key[2048] = { 0x00, };
    uint32_t public_keylength = 0;
    uint8_t private_key[2048] = { 0x00, };
    uint32_t private_keylength = 0;

    memset(&key_param, 0, sizeof(EDGE_ASYM_KEY_PARAM));
    key_param.m_algorithm = EDGE_ASYM_ID_RSA;
    key_param.rsa.m_bits = 2048;
    //key_param.rsa.m_bits = 3072;
    key_param.rsa.m_exponent = 65537;

    printf("\n##======================     rsa_sample() start     ======================##\n");
    do {
        printf("\n *----------------------   edge_asym_gen_keypair()  ------------------------\n");

        ret = edge_asym_gen_keypair(public_key, &public_keylength, private_key, &private_keylength, &key_param);
        if (ret != 0) break;

        hexdump("private_key", private_key, private_keylength);
        hexdump("public_key", public_key, public_keylength);

        param.m_asymAlg = EDGE_ASYM_ID_RSA;
        param.m_hashAlg = EDGE_HASH_ID_SHA256;
        param.rsa.m_signMode = EDGE_RSA_SIGN_MODE_PSS;
        //param.rsa.m_signMode = EDGE_RSA_SIGN_MODE_PKCS1_v15;
        param.rsa.m_mgfHashAlg = EDGE_HASH_ID_SHA256;
        param.rsa.m_saltLength = EDGE_RSA_DEFAULT_SALT_LENGTH;

        hexdump("input", input, inputlength);
        printf("\n *----------------------      edge_asym_sign()      ------------------------\n");
        ret = edge_asym_sign(private_key, private_keylength, &param,input, inputlength, sign, &signlength);
        if (ret != 0)  {
            printf("sign fail %d\n", ret);
			break;
		}
        hexdump("sign", sign, signlength);

        printf("\n *----------------------     edge_asym_verify()     ------------------------\n");
        ret = edge_asym_verify(public_key, public_keylength, &param, input, inputlength, sign, signlength);
        if (ret != 0) {
            printf("sign verify fail %d\n", ret);
            break;
        }
        printf("sign verify success\n");
    } while (0);

    printf("\n##======================      rsa_sample() end      ======================##\n");

    return ret;
}
